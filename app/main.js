...
function renderRows(steps) {
    ...
    for (const step of steps) {
        ...
        const lineDisplay = step.styledLine || step.line;
        let line = step.dir ? `${lineDisplay}<div>${escapeHtml(step.dir)}</div>` : lineDisplay;

        const rowClass = step.line == "WALK" && !!step.graphic ? "walk-row" : "";
        let walk1 = "";
        let walk2 = "";
    
        if (step.line == "WALK") {
            const distanceString = step.distanceInMeters ? ` ${step.distanceInMeters} m` : '';
